...
It shows that the `d` object is 'dataframe'. 
...
Right. It's 'character'. How about the class of the column `Effect`?
...
```{r}
head(d$SeizureOnsetDays == '.')
table(d$SeizureOnsetDays == '.')
table(d$Seizures == "N")
table(d$Seizures, d$SeizureOnsetDays=='.')
```

So we want to subset rows where the seizure phenotype is available.

`d[row,column]`  
`nrow(d)` --- number of rows / ncol  
`dim(d1)` --- dimension of object  
`gsub`('old_one_you_want_to_replace', 'new_one_you_want_to_replace', place)  

```{r}
d1 = d[d$Seizures=='Y',]
min(d1$SeizureOnsetDays)
#> <28 ? we know that this is wrong because there's a 0.
d1$SeizureOnsetDays2 <- d1$SeizureOnsetDays
d1$SeizureOnsetDays2 <- gsub('<', '', d1$SeizureOnsetDays2)
d1$SeizureOnsetDays2 <- as.numeric(d1$SeizureOnsetDays2)
#> as.numeric gets you 'NA' in some info.
#> the angle bracket "<" gets you NA
min(d1$SeizureOnsetDays2)
class(d1$SeizureOnsetDays2)
```
...
For info about tibble, check [here]{https://r4ds.had.co.nz/tibbles.html}

- Check `Type` and `Effect`
```{r}
head(d$Type)
head(d$Effect)
```
...
Do you think this is an effective approach? As we have done in your assignment, `for-loop` is not great choice to process vectors because R can enable `Vectorization` for a shorter and clearer code. So this mean you can apply `gsub` on vector and return your output to another column (could be new assign). 
...
names(c)[1] <- 'p.Protein'
print(c)
#c$Freq와 d$AnyRecurrence의 정보 동일
```
...
```{r}
# Please write a code
e <- as.data.frame(table(d$p.Protein1))
names(e)[1] <- 'p.Protein1'
print(e)
```
```{r}
e_position_recurrence <- merge(d[, c('p.Protein1','p.Protein','AnyRecurrence')], e[, c('p.Protein1','Freq')], all.x = TRUE )
names(e_position_recurrence)[1] <- 'PositionNo'
names(e_position_recurrence)[3] <- 'MutationRecurrence'
names(e_position_recurrence)[4] <- 'PositionRecurrence'
view(e_position_recurrence)
```

```{r}
different_consequence <- filter(e_position_recurrence, PositionRecurrence - MutationRecurrence > 0)
```
