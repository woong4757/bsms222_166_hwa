...
p_selected$`mutation consequence` <- factor(p_selected$`mutation consequence`, levels=rev(levels(p_selected$`mutation consequence`)))


plot <- p_selected %>% ggplot(aes(reorder(`allelic requirement`,allelic_sum), phenotypes, fill = `mutation consequence`)) +
  geom_bar(position = "stack", stat = "identity") +
  coord_flip() +
  geom_label_repel(aes(y = label_ypos, label = phenotypes))+
  theme(legend.position = "top")+
  xlab("allelic requirement") +
  ggtitle("Phenotypic heterogeneity in DDG2P by allelic requirement and mutational consequence")
...
For better readability, I changed `geom_text_repel` to `geom_label_repel` to add color to the numbers. I changed & grouped the data by using `mutate` and several strings of `gsub`. If you look at the code, I grouped _imprinted_, _digenic_, and _mosaic_ in `allelic requirement` into others. They are a bit different from the rest because they affect a bigger range of alleles. Secondly, I put the _X-linked_ mutations together due to their common ground in affecting the X sex chromosome. In `mutation consequence`, I grouped the _5_prime or 3_prime UTR mutation_ and _cis-regulatory or promotor mutation_ into _noncoding mutation_ as they both have mutations in the noncoding region. Also, I changed _activating_ into _gain of function_ as they both mean the same thing in a broad sense. For the _others_, I put _increased gene dosage_ and _part of contiguous gene duplication_ as the former is change in the dosage and latter is a broad scale mutation.
